...
def _declared_types(policy: Mapping[str, Any], field: str, key: str) -> set[str]:
    return {
        str(item.get(key) or "")
        for item in _mappings(policy.get(field))
        if item.get(key)
    }
...
def validate_flow_policy_compatibility(
    *, flow_profile: Mapping[str, Any], policy_registry: Mapping[str, Any]
    ...
    # Exact Flow -> Policy registry binding must match revision/schema/digest.
    policy_bindings = [
        item for item in _mappings(flow_profile.get("registry_bindings"))
        if item.get("registry") == "policy"
    ]
    ...
    # Static compatibility proves declarations cannot be weaker than minimums.
    ...
    compiled_declared = None
    if isinstance(flow_profile.get("compiled"), Mapping):
        compiled_declared = flow_profile["compiled"].get("workflow_digest")
