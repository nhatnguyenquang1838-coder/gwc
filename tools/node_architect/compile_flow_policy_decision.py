...
def _gate_policy_ref(flow_profile: Mapping[str, Any], gate: str) -> str | None:
    ...
    if not isinstance(workflow, Mapping):
        return None
    matches = [item for item in workflow.get("gate_bindings", []) if isinstance(item, Mapping) and item.get("gate") == gate]
    if len(matches) != 1:
        return None
...
def _policy(policy_registry: Mapping[str, Any], policy_ref: str | None) -> Mapping[str, Any] | None:
    matches = [item for item in policy_registry.get("policies", []) if isinstance(item, Mapping) and item.get("id") == policy_ref]
...
def _known_nodes(flow_profile: Mapping[str, Any]) -> set[str]:
    ...
    nodes.update(str(item) for item in workflow.get("entry_nodes", []) if item)
...
def compile_flow_policy_decision(
    *, flow_profile: Mapping[str, Any], policy_registry: Mapping[str, Any],
    current_node: str, current_gate: str,
    applicability_decision: Mapping[str, Any], context: Mapping[str, Any],
    next_nodes: Sequence[str] | None = None, next_gate: str | None = None,
