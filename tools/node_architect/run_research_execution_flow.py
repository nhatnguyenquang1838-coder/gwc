...
def _result(payload: Mapping[str, Any], cp: Mapping[str, Any], outcome: str, reason: str, **extra: Any) -> dict[str, Any]:
    value = {
        "schema_version": "1.0", "artifact_type": "research-execution-flow-decision",
        "run_id": str(payload["run_id"]), "dispatch_id": str(payload["dispatch_id"]),
        "trigger_mode": str(payload["trigger_mode"]), "outcome": outcome, "reason_code": reason,
        "checkpoint": dict(cp), "parallel_execution_allowed": False, "g4_g5_g6_authority_granted": False,
        **extra,
    }
...
def run_research_execution_flow(payload: Mapping[str, Any]) -> dict[str, Any]:
    run_id, dispatch_id = str(payload.get("run_id", "")).strip(), str(payload.get("dispatch_id", "")).strip()
    ...
    if isinstance(stop, Mapping):
        for field, code in (("terminal_blocker", "TERMINAL_BLOCKER"), ("human_authority_required", "HUMAN_AUTHORITY_REQUIRED"), ("approval_expired", "RESEARCH_APPROVAL_EXPIRED"), ("scope_drift", "RESEARCH_SCOPE_DRIFT")):
            if stop.get(field) is True:
                cp = {**cp, "revision": int(cp["revision"]) + 1, "stop_reason": code}
                return _result(payload, cp, "STOPPED", code)

    # Duplicate dispatch is fenced only after an external-effect phase has started.
    if cp.get("active_dispatch_id") == dispatch_id and cp.get("phase") in {"WAIT_PROJECTIONS", "WAIT_CLAIM", "HANDOFF_GWC", "WAIT_G3", "HUMAN_G4"}:
    ...
    if cp["phase"] == "SELECT_RESEARCH":
        selector_input = {
            "run_id": run_id, "trigger_mode": trigger_mode, "active_lane": payload.get("active_lane"),
            "excluded_lanes": payload.get("excluded_lanes", []), "research_records": records,
            "approvals": approvals, "dependency_evidence": payload.get("dependency_evidence", {}),
        }
        ...
        cp = {**cp, "revision": int(cp["revision"]) + 1, "active_research_ref": ref, "active_approval_id": approval_id, "active_dispatch_id": dispatch_id, "phase": "MATERIALIZE", "stop_reason": None}

    by_ref = {str(x.get("research_ref")): x for x in records if isinstance(x, Mapping) and x.get("research_ref")}
    approvals_by_id = {str(x.get("approval_id")): x for x in approvals if isinstance(x, Mapping) and x.get("approval_id")}
    ...
    if cp["phase"] in {"MATERIALIZE", "WAIT_PROJECTIONS"}:
        materialization = materialize_research_execution(research, approval, payload.get("projection_readbacks", {}), effects_started=cp.get("effects_started", []))
        if materialization["outcome"] == "CONFLICT":
            return _result(payload, cp, "STOPPED", materialization["reason_code"], materialization=materialization)
        if materialization["outcome"] == "RECONCILIATION_REQUIRED":
            return _result(payload, cp, "WAITING", materialization["reason_code"], materialization=materialization)
        if materialization["outcome"] == "ACTION_REQUIRED":
            providers = [x["provider"] for x in materialization["projection_intents"]]
            cp = {**cp, "revision": int(cp["revision"]) + 1, "materialization_key": materialization["materialization_key"], "phase": "WAIT_PROJECTIONS", "effects_started": sorted(set(cp.get("effects_started", [])) | set(providers)), "active_dispatch_id": dispatch_id}
            return _result(payload, cp, "ACTION_REQUIRED", "EXECUTION_PROJECTION_CREATE_REQUIRED", materialization=materialization, external_actions=materialization["projection_intents"])
        cp = {**cp, "revision": int(cp["revision"]) + 1, "materialization_key": materialization["materialization_key"], "phase": "WAIT_CLAIM", "effects_started": sorted(set(cp.get("effects_started", [])) | {"github", "jira"}), "active_dispatch_id": dispatch_id}
        if not isinstance(payload.get("claim_readback"), Mapping):
            return _result(payload, cp, "ACTION_REQUIRED", "EXECUTION_TASK_CLAIM_REQUIRED", materialization=materialization, external_actions=[materialization["claim_intent"]])

    claim = payload.get("claim_readback")
    if cp["phase"] == "WAIT_CLAIM":
        if not isinstance(claim, Mapping):
            materialization = materialize_research_execution(research, approval, payload.get("projection_readbacks", {}), effects_started=[])
            return _result(payload, cp, "ACTION_REQUIRED", "EXECUTION_TASK_CLAIM_REQUIRED", materialization=materialization, external_actions=[materialization["claim_intent"]])
        ...
        cp = {**cp, "revision": int(cp["revision"]) + 1, "execution_task_ids": claim.get("execution_task_ids"), "phase": "HANDOFF_GWC"}

    if cp["phase"] == "HANDOFF_GWC":
        materialization = materialize_research_execution(research, approval, payload.get("projection_readbacks", {}), effects_started=[])
        spec = materialization["execution_task_spec"]
        handoff = {
            "action": "enter_existing_gwc_task_runtime", "task_ids": cp["execution_task_ids"],
            "materialization_key": cp["materialization_key"], "execution_task_spec_digest": spec["spec_digest"],
            "g2_authority": spec["child_authority"]["g2"], "g3_authority": spec["child_authority"]["g3"],
            "required_runtime_invariants": ["MODE_DOES_NOT_BYPASS_NODE_RUNTIME", "EXACT_HEAD_CI_REQUIRED", "REVIEW_STALE_AFTER_HEAD_CHANGE"],
        ...
        cp = {**cp, "revision": int(cp["revision"]) + 1, "phase": "HUMAN_G4", "stop_reason": "HUMAN_AUTHORITY_REQUIRED"}
        return _result(payload, cp, "HUMAN_REQUIRED", "HUMAN_AUTHORITY_REQUIRED", g4_request={"gate":"G4_MERGE","task_ids":cp["execution_task_ids"],"pr_number":runtime.get("pr_number"),"head_sha":runtime.get("head_sha"),"scope_hash":runtime.get("scope_hash")})
