...
def _ci_evidence(request: ClientRuntimeRequest, state: RuntimeState) -> Mapping[str, Any]:
    ...
    result = capture_ci_evidence(
        ...
        checkpoint_store=state.setdefault(
            "ci_checkpoint_store",
            {"schema_version": "1.0", "artifact_type": "runtime-checkpoint-store", "revision": 0, "events": [], "checkpoints": {}},
        ),
...
def _quality(request: ClientRuntimeRequest, state: RuntimeState) -> Mapping[str, Any]:
    ...
    result = check_evidence_quality(payload, replay_cache=state.setdefault("quality_replay_cache", {}))
...
def _g3(request: ClientRuntimeRequest, state: RuntimeState) -> Mapping[str, Any]:
    ...
    result = decide_g3_pass(payload, replay_cache=state.setdefault("g3_replay_cache", {}))
...
def _event(state: RuntimeState, node: str, outcome: Mapping[str, Any]) -> dict[str, Any]:
    event = {
        ...
        "evidence_digest": outcome.get("evidence_digest") or outcome.get("quality_digest") or outcome.get("decision_digest"),
    }
    ...
    for node in request.route_nodes:
        ...
        if node == "validation_quality.ci-evidence-capture" and outcome.get("status") != PASS:
            if outcome.get("checkpoint_required") and outcome.get("checkpoint_key"):
                state["checkpoints"].append({"node": node, "checkpoint_key": outcome["checkpoint_key"], "evidence_digest": outcome.get("evidence_digest")})
...
__all__ = [
    "ALLOWED_ROUTE_INTENT", "BLOCKED", "BLOCKED_MISSING_REQUIRED_FIELD",
    "BLOCKED_NODE_HANDLER_ERROR", "BLOCKED_NODE_HANDLER_UNAVAILABLE",
    "BLOCKED_ROUTE_NOT_ALLOWLISTED", "ClientRuntimeRequest", "ClientRuntimeResult",
    "PASS", "TERMINAL_NODE", "TERMINAL_PASS", "VERTICAL_SLICE_ROUTE",
    "default_handler_registry", "normalize_request", "run_client_runtime", "validate_route",
]
