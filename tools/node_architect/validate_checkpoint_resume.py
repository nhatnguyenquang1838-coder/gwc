...
def validate_resume_token(
    ...
    *,
    ...
    try:
        ...
        # Tokens generated by the instruction-backed M5 node carry a complete
        # binding. Keep legacy 0.1 fixtures readable, but validate the expanded
        # contract whenever any expanded binding field is present.
        expanded = any(key in resume_token for key in (
            "task_id", "run_id", "node_id", "scope_hash", "state_digest",
            "lease_token", "fencing_token", "token_digest",
        ))
        if expanded:
            for key in ("task_id", "run_id", "node_id", "state_digest", "lease_token", "fencing_token"):
                require_string(resume_token, key)
            scope_hash = require_string(resume_token, "scope_hash")
            if not re.fullmatch(r"sha256:[0-9a-f]{64}", scope_hash):
                raise ValidationError("resume_token.scope_hash: invalid digest")
            for key in ("base_sha", "head_sha"):
                value = resume_token.get(key)
                if key == "head_sha" and value is None:
                    continue
                if not isinstance(value, str) or not re.fullmatch(r"[0-9a-f]{40}", value):
                    raise ValidationError(f"resume_token.{key}: invalid SHA")
            state_digest = resume_token["state_digest"]
            if not re.fullmatch(r"sha256:[0-9a-f]{64}", state_digest):
                raise ValidationError("resume_token.state_digest: invalid digest")
            if checkpoint.get("state_digest") not in {None, state_digest}:
                raise ValidationError("resume_token.state_digest does not match checkpoint")
            if resume_token.get("gate") != "G2_EXECUTION":
                raise ValidationError("resume_token.gate: expected G2_EXECUTION")
            if resume_token.get("token_digest") != digest({key: value for key, value in resume_token.items() if key != "token_digest"}):
                raise ValidationError("resume_token.token_digest does not match token")
            authority_flags = (
            ...
            if any(resume_token.get(key) is not False for key in authority_flags):
                raise ValidationError("resume token authority flags must all be false")
