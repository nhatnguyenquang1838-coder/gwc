...
def _identity(payload: Mapping[str, Any]) -> dict[str, str]:
    return {field: str(payload.get(field, "")).strip() for field in (
        "task_id", "repository", "branch", "base_sha", "head_sha", "scope_hash",
        "graph_revision", "policy_digest", "idempotency_key",
    )}
...
def decide_g3_pass(evidence: Mapping[str, Any], *, replay_cache: MutableMapping[str, dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    if replay_cache is not None and cache_key and cache_key in replay_cache:
        ...
        if cached.get("input_digest") == input_digest:
            ...
            return replay
        return {
            "schema_version": "1.0", "artifact_type": "g3-pass-decision", "node_id": NODE_ID,
            **identity, "outcome": G3_BLOCKED, "reason_codes": ["EVIDENCE_REJECTED", "G3_BLOCKED"],
            "input_digest": input_digest,
            "decision_digest": _digest({"identity": identity, "outcome": G3_BLOCKED, "conflict": True}),
            "transition_effect_applied": False, "replayed": False, **_authority_boundary(),
        }
    ...
    decision_basis = {
        "identity": identity, "outcome": outcome, "reason_codes": reason_codes,
        "quality_digest": quality.get("quality_digest"),
        "validation_digests": [item.get("digest") for item in validations if isinstance(item, Mapping)],
        "ready_for_review": dict(ready),
    }
    result = {
        "schema_version": "1.0", "artifact_type": "g3-pass-decision", "node_id": NODE_ID,
        **identity, "outcome": outcome, "reason_codes": reason_codes,
        "input_digest": input_digest, "decision_digest": _digest(decision_basis),
        "transition_effect_applied": outcome == G3_PASS, "replayed": False, **_authority_boundary(),
    }
