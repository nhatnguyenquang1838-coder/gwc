...
def derive_g2_authority(
    ...
    *,
    ...
    requested_paths = list(request.get("requested_paths", []))
    requested_actions = list(request.get("requested_actions", []))
    if not requested_paths or not set(requested_paths).issubset(set(task.get("authorized_paths", []))):
        return _deny("G2_EXECUTION", "AUTONOMOUS_SCOPE_DRIFT", run_id=run_id, task_id=task_id)
    if not requested_actions or not set(requested_actions).issubset(set(task.get("authorized_g2_actions", []))):
...
def derive_g4_receipt(
    ...
    *,
    ...
    if validation["outcome"] != "PASS":
        return _deny("G4_MERGE", validation["reason_codes"][0], run_id=str(manifest.get("run_id", "")), task_id=task_id or None)
    task = _task(manifest, task_id)
    if task is None:
        return _deny("G4_MERGE", "AUTONOMOUS_TASK_NOT_ALLOWLISTED", run_id=str(manifest["run_id"]), task_id=task_id or None)
    if context.get("target_branch") == "main":
        return _deny("G4_MERGE", "AUTONOMOUS_MAIN_TARGET_FORBIDDEN", run_id=str(manifest["run_id"]), task_id=task_id)
    if context.get("target_branch") != "pre-prod" or context.get("target_branch") != manifest.get("target_branch"):
        return _deny("G4_MERGE", "AUTONOMOUS_PREPROD_TARGET_REQUIRED", run_id=str(manifest["run_id"]), task_id=task_id)
    if context.get("task_scope_hash") != task.get("scope_hash"):
        return _deny("G4_MERGE", "AUTONOMOUS_SCOPE_DRIFT", run_id=str(manifest["run_id"]), task_id=task_id)
    head = str(context.get("approved_head_sha", ""))
    if not SHA_RE.fullmatch(head):
        return _deny("G4_MERGE", "AUTONOMOUS_HEAD_DRIFT", run_id=str(manifest["run_id"]), task_id=task_id)
    for field, code in (
        ...
        if not DIGEST_RE.fullmatch(str(context.get(field, ""))):
            return _deny("G4_MERGE", code, run_id=str(manifest["run_id"]), task_id=task_id)
    pr_number = context.get("pr_number")
    if not isinstance(pr_number, int) or pr_number < 1:
        return _deny("G4_MERGE", "AUTONOMOUS_STANDING_G4_RECEIPT_INVALID", run_id=str(manifest["run_id"]), task_id=task_id)
