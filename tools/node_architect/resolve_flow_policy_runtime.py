...
def _participant_gate(flow_profile: Mapping[str, Any], node: str) -> str | None:
    ...
    if not isinstance(workflow, Mapping):
        return None
    matches = [
        item for item in workflow.get("participants", [])
        if isinstance(item, Mapping) and item.get("participant") == node
    ]
    if len(matches) != 1:
        return None
    gate = matches[0].get("gate")
...
def _edge_selected(edge: Mapping[str, Any], context: Mapping[str, Any]) -> bool:
    ...
    if kind in {"retry", "compensate"}:
        return context.get("transition_kind") == kind
    if kind in {"blocked", "human_required", "terminal"}:
        return context.get("transition_kind") == kind
    return False
...
def _next_step(flow_profile: Mapping[str, Any], current_node: str, context: Mapping[str, Any]) -> tuple[list[str], str | None, list[str]]:
    ...
    outgoing = [
        ...
        if isinstance(edge, Mapping) and edge.get("source") == current_node and _edge_selected(edge, context)
    ]
    ...
    *, runtime_profile: Mapping[str, Any], flow_profile: Mapping[str, Any],
    ...
    if activation.get("outcome") != "ACTIVATABLE":
        payload = {
            ...
            "outcome": "BLOCKED",
            "reason_codes": list(activation.get("reason_codes", ["RUNTIME_NOT_ACTIVATABLE"])),
