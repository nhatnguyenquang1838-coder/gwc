...
Protected-base GWC evidence
        │
        ▼
Standalone Task-Me scan
        │
        ├── per-task planning package
        ├── planning/traceability graph
        └── Kiro requirements/design/tasks projection
        │
        ▼
GWC G0/G1 validation
        │
        ▼
Bounded G2 implementation tasks
        │
        ▼
Registry validation + Cytoscape v3 binding
```
...
```text
Current-state audit ─────┐
Authority matrix ────────┼→ Blocker resolution → Registries + v3 binding
Schema design ───────────┘
```
