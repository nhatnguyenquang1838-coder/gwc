...
Implement SCRUM-176 in dependency order while keeping the change inside the
...
```mermaid
flowchart TD
  T1[Freeze exact source evidence and scope] --> T2[Extend source-resolution contract]
...
- [ ] 1. Freeze the exact source evidence and current scope
  - Confirm the active repository head and parent context for `SCRUM-176`.
  - Re-read `source-resolution.node.json`, the family README, validator, tests,
    runtime graph, node registry, and the current GWC maturity task before
    editing.
  - Record the scope as `intake_context.source-resolution`, not the entire
    family.
  - Requirements: Requirement 1, Requirement 3, Requirement 4.

- [ ] 2. Extend the `source-resolution` contract in place
  - Add typed source-mode, source-authority, provenance, and reason-code
    fields to `core/node-architect/node-catalog/intake_context/source-resolution.node.json`.
...
- [ ] 3. Update the family validator and coherence checks
  - Extend `tools/node_architect/validate_node_catalog_intake_context.py` to
    accept the typed source-resolution contract.
...
- [ ] 4. Expand regression tests
  - Update `tests/test_node_catalog_intake_context.py` with repository-only,
    package-only, mixed-source, ambiguous-source, malformed-input, and
    provenance-evidence cases.
...
- [ ] 5. Refresh documentation and exact-head evidence artifacts
  - Update `core/node-architect/node-catalog/intake_context/README.md` only as
    needed to explain the typed source-resolution contract.
