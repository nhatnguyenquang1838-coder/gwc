import copy
import unittest
from tools.node_architect.materialize_research_execution import compile_execution_task_spec, materialize_research_execution, validate_research_approval
...
class MaterializationTests(unittest.TestCase):
    def test_deterministic_spec_and_no_g4(self):
        r,a=research_and_approval(); s1=compile_execution_task_spec(r,a,now=NOW); s2=compile_execution_task_spec(r,a,now=NOW)
        self.assertEqual(s1["materialization_key"],s2["materialization_key"]); self.assertFalse(s1["g4_g5_g6_authority_granted"]); self.assertTrue(s1["child_authority"]["g2"]["authority_granted"]); self.assertTrue(s1["child_authority"]["g3"]["authority_granted"])
    def test_g3_omitted_stays_none(self):
        r,a=research_and_approval(g3=False); s=compile_execution_task_spec(r,a,now=NOW); self.assertIsNone(s["child_authority"]["g3"])
    def test_scope_drift_blocks(self):
        r,a=research_and_approval(); r=copy.deepcopy(r); r["scopes"]["round-1"]["objective"]="drift"
        ok,reason=validate_research_approval(r,a,now=NOW); self.assertFalse(ok); self.assertEqual(reason,"RESEARCH_SCOPE_DRIFT")
    def test_authority_escalation_blocks(self):
        r,a=research_and_approval(); a=copy.deepcopy(a); a["delegated_g3_actions"].append("merge")
        ok,reason=validate_research_approval(r,a,now=NOW); self.assertFalse(ok); self.assertEqual(reason,"RESEARCH_APPROVAL_AUTHORITY_ESCALATION")
    def test_reconcile_both_missing_one_and_conflict(self):
        r,a=research_and_approval(); first=materialize_research_execution(r,a,{},now=NOW); self.assertEqual(first["outcome"],"ACTION_REQUIRED"); self.assertEqual({x["provider"] for x in first["projection_intents"]},{"github","jira"})
        key=first["materialization_key"]; gh={"id":"324","materialization_key":key,"origin_research_ref":"SCRUM-279"}
        one=materialize_research_execution(r,a,{"github":[gh],"jira":[]},now=NOW); self.assertEqual([x["provider"] for x in one["projection_intents"]],["jira"])
        jr={"id":"SCRUM-285","materialization_key":key,"origin_research_ref":"SCRUM-279"}
        ready=materialize_research_execution(r,a,{"github":[gh],"jira":[jr]},now=NOW); self.assertEqual(ready["outcome"],"READY"); self.assertIsNotNone(ready["claim_intent"])
        conflict=materialize_research_execution(r,a,{"github":[gh,{**gh,"id":"325"}],"jira":[jr]},now=NOW); self.assertEqual(conflict["outcome"],"CONFLICT")
    def test_partial_effect_requires_reconciliation(self):
        r,a=research_and_approval(); x=materialize_research_execution(r,a,{"github":[],"jira":[]},effects_started=["github"],now=NOW); self.assertEqual(x["outcome"],"RECONCILIATION_REQUIRED")
