...
BASE_SHA = "a" * 40
G2_ACTIONS = [
    "create_guarded_branch_or_worktree", "modify_approved_files", "run_sandboxed_validation",
    "stage", "create_commit", "push_working_branch",
]


def policy() -> dict:
    return {
        "schema_version": "1.0", "artifact_type": "autonomous-preprod-run-policy",
        "policy_id": "AUTONOMOUS_PREPROD_INTEGRATION_POLICY", "policy_revision": "test-v1",
        "repository": "nhatnguyenquang1838-coder/gwc", "target_branch": "pre-prod",
        "allowed_branch_prefix": "auto/", "max_child_risk": "R2",
        "allowed_g2_actions": list(G2_ACTIONS), "allowed_g4_actions": ["merge_approved_pr"],
        "denied_actions": [
        ...
        "issued_at": "2026-08-07T00:00:00Z", "expires_at": "2026-08-08T00:00:00Z",
    }
...
def manifest(p: dict | None = None, *, tasks: list[dict] | None = None,
    ...
    p = p or policy()
    value = {
        "schema_version": "1.0", "artifact_type": "autonomous-preprod-run-manifest",
        "run_id": "run-test-1", "repository": p["repository"], "policy_id": p["policy_id"],
        "policy_revision": p["policy_revision"], "policy_digest": canonical_digest(p),
        "approved_base_ref": "main", "approved_base_sha": BASE_SHA, "target_branch": "pre-prod",
        "allowed_tasks": tasks or [task()], "issued_at": "2026-08-07T01:00:00Z",
        "expires_at": "2026-08-07T23:00:00Z", "idempotency_key": "run-test-1-idempotency",
    }
    ...
    value["authority_receipt"] = {
        "status": "present", "source": "github_actions_bot_comment", "bot_login": "github-actions[bot]",
        "marker": "gwc:autonomous-preprod-run-authority-receipt", "approval_id": "APPROVE_AUTONOMOUS_RUN_TEST_1",
        "receipt_comment_id": 900101, "source_comment_id": 900100, "approved_run_id": value["run_id"],
        "approved_policy_id": value["policy_id"], "approved_policy_revision": value["policy_revision"],
        "approved_policy_digest": value["policy_digest"], "manifest_scope_digest": scope_digest,
        "scope_hash_prefix": scope_digest.removeprefix("sha256:")[:16],
        "issued_at": "2026-08-07T01:05:00Z", "expires_at": "2026-08-07T23:30:00Z",
    }
    return value
...
class AutonomousPreprodPolicyTests(unittest.TestCase):
    ...
    def test_explicit_active_authority_self_modification_is_blocked(self):
        path = "tools/node_architect/validate_autonomous_preprod_policy.py"
        p = policy(); m = manifest(p, tasks=[task(paths=[path])], immutable_paths=[path])
        rebind_parent_receipt(m)
        result = validate_manifest(p, m, root=ROOT, now=NOW)
        self.assertIn("AUTONOMOUS_ACTIVE_AUTHORITY_SELF_MODIFICATION_FORBIDDEN", result["reason_codes"])
    ...
    def test_project_instruction_is_still_blocked_when_explicitly_active(self):
    ...
    def test_path_traversal_fails_scope(self):
        p = policy(); m = manifest(p, tasks=[task(paths=["tools/node_architect/../node_architect/x.py"])])
