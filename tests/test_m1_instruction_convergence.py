...
class M1InstructionConvergenceTests(unittest.TestCase):
    ...
    def test_fresh_instruction_package_declares_universal_default_and_compatibility_boundary(self):
        ...
        slack_protocol = (ROOT / "agents/shared/slack-controller-executor-protocol.md").read_text(encoding="utf-8")
        ...
        self.assertNotIn("When ChatGPT acts as Controller for a Slack-mediated Executor run, the following additive instruction chain is **mandatory**", chatgpt)
        self.assertIn("validated current `RunState`/`RuntimePlan` route ID and adapter identity", chatgpt)
        self.assertNotIn("When ChatGPT is acting as Controller for a Slack-mediated Executor run, the following additive instruction chain is **mandatory**", agent_index)
        self.assertIn("validated current `RunState`/`RuntimePlan` route ID and adapter identity", agent_index)
        self.assertIn("Universal Run source outcomes", slack_protocol)
        self.assertIn("current legacy Slack adapter accepts only", slack_protocol)
        self.assertIn("`CONTINUE | WAIT_CONTROLLER | TERMINAL`", slack_protocol)
        self.assertIn("RETRY`, `REPAIR`, and `REPLAN` are not supported by the", slack_protocol)
        self.assertIn("unsupported compatibility projection", slack_protocol)
