...
class ScaleControlTests(unittest.TestCase):
    def test_repository_family_passes(self):
        ...
        validate_family(root / "core/node-architect/node-catalog/scale_control", root / "projects/gwc/package.yaml", root / "core/node-architect/failure-simulation-matrix.json")

    def test_exactly_nine_semantics(self):
        root = Path(__file__).resolve().parents[1]
        stems = {p.name.removesuffix(".node.json") for p in (root / "core/node-architect/node-catalog/scale_control").glob("*.node.json")}
        self.assertEqual(stems, REQUIRED_SEMANTICS)

    def test_gate_partition(self):
        root = Path(__file__).resolve().parents[1]
        nodes = [json.loads(p.read_text()) for p in (root / "core/node-architect/node-catalog/scale_control").glob("*.node.json")]
    ...
    def test_catalog_exports_eighty_one_nodes(self):
        ...
        node_paths = [i["path"] for i in package["instructions"] if "/node-catalog/" in i["path"] and i["path"].endswith(".node.json")]
