...
def test_effectful_step_rejects_none_requested_action():
    """seq=11 M4: effectful step with requested_action=None fails closed."""
    step = _plan_step("commit", allowed_actions=("commit",))
    plan = _plan({"commit": step})
    authority = _authority(allowed_actions=["commit"], gate="G3_PR")
    executor = PlanBoundRuntimeExecutor(plan=plan, authority=authority)
    with pytest.raises(PlanBoundRuntimeError, match="requested_action missing"):
        executor.execute_step("commit", {}, authority=authority, requested_action=None)


def test_effectful_step_rejects_empty_requested_action():
    """seq=11 M4: effectful step with requested_action='' (empty) fails closed."""
    step = _plan_step("commit", allowed_actions=("commit",))
    plan = _plan({"commit": step})
    authority = _authority(allowed_actions=["commit"], gate="G3_PR")
    executor = PlanBoundRuntimeExecutor(plan=plan, authority=authority)
...
def test_read_only_step_allows_none_requested_action():
    """seq=11 M4: read-only step with requested_action=None is permitted."""
    step = _plan_step("inspect", allowed_actions=("read",))
