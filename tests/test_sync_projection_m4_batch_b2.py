...
from tools.node_architect.ds_admin_state_projection import project_ds_admin_state

SCHEMA = json.loads(Path("schemas/ds-admin-state-projection.schema.json").read_text(encoding="utf-8"))
...
def valid_source_authority():
    decision = {
        ...
        "source_bindings": [
            {"source_type": "TASK_RECORD", "authority_class": "CANONICAL", "ref": "jira:SCRUM-220",
             "revision": REVISION_A, "content_digest": DIGEST_A, "observed_at": "2026-08-06T13:00:00Z", "status": "VERIFIED"},
        ],
        "field_authority": [
            {"field_path": "/task/status", "source_ref": "jira:SCRUM-220", "source_revision": REVISION_A,
...
def valid_linkset(source_digest=DIGEST_A):
    return {
        ...
        "links": [
            {"evidence_id": "task-220", "source_type": "TASK_RECORD", "ref": "jira:SCRUM-220",
...
def valid_envelope(source_digest=DIGEST_A):
    return {
        ...
        "canonical_state": {
            ...
            "task_title": "DS Admin state projection",
            "task_type": "Subtask",
...
class DSAdminStateProjectionTests(unittest.TestCase):
    ...
    def build(self, **overrides):
        ...
        result = project_ds_admin_state(**payload)
        self.assert_schema_valid(result)
        return result

    def test_valid_projection_is_ready_and_read_only(self):
        result = self.build()
        self.assertEqual(result["outcome"], "READY")
        self.assertEqual(result["reason_code"], "DS_ADMIN_PROJECTION_READY")
    ...
    def test_invalid_source_authority_is_blocked(self):
        ...
        bad["outcome"] = "BLOCKED"
        self.assertEqual(self.build(source_authority_decision=bad)["reason_code"], "DS_ADMIN_SOURCE_AUTHORITY_INVALID")

    def test_invalid_evidence_linkset_is_blocked(self):
        bad = valid_linkset()
        bad["outcome"] = "BLOCKED"
        self.assertEqual(self.build(evidence_linkset=bad)["reason_code"], "DS_ADMIN_EVIDENCE_LINKSET_INVALID")

    def test_invalid_privacy_boundary_is_blocked(self):
        bad = valid_privacy()
        bad["outcome"] = "BLOCKED"
        self.assertEqual(self.build(privacy_boundary_decision=bad)["reason_code"], "DS_ADMIN_PRIVACY_BOUNDARY_INVALID")

    def test_source_authority_digest_mismatch_is_blocked(self):
        env = valid_envelope()
        env["source_authority_digest"] = DIGEST_B
        self.assertEqual(self.build(envelope=env)["reason_code"], "DS_ADMIN_SOURCE_AUTHORITY_INVALID")

    def test_prior_binding_to_other_task_is_blocked(self):
        prior = self.build()
        prior["task_id"] = "SCRUM-221"
        self.assertEqual(self.build(prior_projection=prior)["reason_code"], "DS_ADMIN_PRIOR_BINDING_MISMATCH")

    def test_revision_regression_is_blocked(self):
        prior = self.build()
        prior["canonical_state"] = {**prior["canonical_state"], "repository_head": REVISION_B}
        self.assertEqual(self.build(prior_projection=prior)["reason_code"], "DS_ADMIN_REVISION_REGRESSION")

    def test_prior_readback_mismatch_is_blocked(self):
        prior = self.build()
        prior["canonical_state"] = {**prior["canonical_state"], "task_status": "Done"}
        self.assertEqual(self.build(prior_projection=prior)["reason_code"], "DS_ADMIN_PRIOR_READBACK_MISMATCH")

    def test_prior_readback_match_is_noop_current(self):
        prior = self.build()
        result = self.build(prior_projection=prior)
        self.assertEqual(result["reason_code"], "DS_ADMIN_PROJECTION_CURRENT")
    ...
    def test_disallowed_canonical_key_is_blocked(self):
        ...
        self.assertEqual(self.build(envelope=env)["reason_code"], "DS_ADMIN_INPUT_INVALID")
    ...
    def test_invalid_task_id_is_blocked(self):
        self.assertEqual(self.build(task_id="bad")["reason_code"], "DS_ADMIN_INPUT_INVALID")
