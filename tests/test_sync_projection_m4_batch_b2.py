...
from tools.node_architect.external_audit_event_projection import project_external_audit_event

SCHEMA = json.loads(Path("schemas/external-audit-event-projection.schema.json").read_text(encoding="utf-8"))
...
def valid_source_authority():
    decision = {
        ...
        "projection_target": "external-audit",
        "source_bindings": [
            {"source_type": "TASK_RECORD", "authority_class": "CANONICAL", "ref": "jira:SCRUM-222",
             "revision": REVISION_A, "content_digest": DIGEST_A, "observed_at": "2026-08-06T13:00:00Z", "status": "VERIFIED"},
        ],
        "field_authority": [
            {"field_path": "/task/status", "source_ref": "jira:SCRUM-222", "source_revision": REVISION_A,
...
def valid_linkset(source_digest=DIGEST_A):
    return {
        ...
        "links": [
            {"evidence_id": "task-222", "source_type": "TASK_RECORD", "ref": "jira:SCRUM-222",
...
class ExternalAuditEventProjectionTests(unittest.TestCase):
    ...
    def build(self, **overrides):
        ...
        result = project_external_audit_event(**payload)
        self.assert_schema_valid(result)
        return result

    def test_valid_event_is_ready_and_read_only(self):
        result = self.build()
    ...
    def test_invalid_source_authority_is_blocked(self):
        ...
        bad["outcome"] = "BLOCKED"
        self.assertEqual(self.build(source_authority_decision=bad)["reason_code"], "EXTERNAL_AUDIT_SOURCE_AUTHORITY_INVALID")

    def test_invalid_evidence_linkset_is_blocked(self):
        bad = valid_linkset()
        bad["outcome"] = "BLOCKED"
        self.assertEqual(self.build(evidence_linkset=bad)["reason_code"], "EXTERNAL_AUDIT_EVIDENCE_LINKSET_INVALID")

    def test_invalid_privacy_boundary_is_blocked(self):
        bad = valid_privacy()
        bad["outcome"] = "BLOCKED"
        self.assertEqual(self.build(privacy_boundary_decision=bad)["reason_code"], "EXTERNAL_AUDIT_PRIVACY_BOUNDARY_INVALID")

    def test_source_authority_digest_mismatch_is_blocked(self):
        env = valid_envelope()
        env["source_authority_digest"] = DIGEST_B
        self.assertEqual(self.build(envelope=env)["reason_code"], "EXTERNAL_AUDIT_SOURCE_AUTHORITY_INVALID")

    def test_prior_binding_to_other_task_is_blocked(self):
        prior = self.build()
        prior["task_id"] = "SCRUM-221"
        self.assertEqual(self.build(prior_projection=prior)["reason_code"], "EXTERNAL_AUDIT_PRIOR_BINDING_MISMATCH")

    def test_prior_readback_mismatch_is_blocked(self):
        prior = self.build()
        prior["canonical_state"] = {**prior["canonical_state"], "repository_head": REVISION_B}
        self.assertEqual(self.build(prior_projection=prior)["reason_code"], "EXTERNAL_AUDIT_PRIOR_READBACK_MISMATCH")

    def test_prior_readback_match_is_noop_current(self):
        prior = self.build()
        result = self.build(prior_projection=prior)
        self.assertEqual(result["reason_code"], "EXTERNAL_AUDIT_EVENT_CURRENT")
    ...
    def test_disallowed_canonical_key_is_blocked(self):
        ...
        self.assertEqual(self.build(envelope=env)["reason_code"], "EXTERNAL_AUDIT_INPUT_INVALID")
    ...
    def test_invalid_task_id_is_blocked(self):
        self.assertEqual(self.build(task_id="bad")["reason_code"], "EXTERNAL_AUDIT_INPUT_INVALID")
